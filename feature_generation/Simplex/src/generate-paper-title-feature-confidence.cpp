...
map<int, double> idf;
map<int, map<int, double> > confidence_score;
...
void compute_author_vector(int author_id)
{
    ...
    for (map<int, double>::iterator it = acc.begin(); it != acc.end(); it++)
        v->push_back(VSMElement(it->first, it->second));

}
inline double inner_product(vector<VSMElement>& va, vector<VSMElement>& vb, int author_id, int paper_id)
{
    vector<VSMElement>::iterator ita = va.begin(), itb = vb.begin(), enda = va.end(), endb = vb.end();
    double confidence = 1.0 - confidence_score[author_id][paper_id];
    ...
    while (ita != enda && itb != endb)
    {
        ...
        {
            sum += ita->weight * itb->weight * idf[ita->index] * confidence;
            ita++;
            itb++;
        }
...
{
    double score = inner_product(author_vector[author_id], paper_title[paper_id], author_id, paper_id);
...
int main(int argc, char *argv[])
{
    if (argc != 1 + 8)
    {
        fprintf(stderr, "Usage : %s PaperAuthor.csv PaperTitle PaperTitleStat Train.csv Test.csv confidence_score train_output test_outputfile\n", argv[0]);
        return -1;
    }
    ...
    FILE *fptr_confidence_score;
    fptr_confidence_score = fopen(argv[6], "r");
    assert(fptr_confidence_score != NULL);

    FILE *fptr_train_output;
    fptr_train_output = fopen(argv[7], "w+");
    assert(fptr_train_output != NULL);


    FILE *fptr_test_output;
    fptr_test_output = fopen(argv[8], "w+");
    ...
    printf("Loading confidence score\n");
    assert(load_confidence_score(fptr_confidence_score, confidence_score) == 0);
